...
def say_hi(name):
    """
    Print 'Hi' followed by the name provided as the input.
    """
    print "Hi " + name
...
def print_product(integer_1, integer_2):
    """
    Print the product of two integers provided as input.
    """
...
def repeat_string(string, times_to_repeat):
    """
    Print string 'times_to_repeat' times.
    """
    print string * times_to_repeat
...
def print_sign(integer):
    """
    Print a statement indicating if the input integer is greater than, less
    than, or equal to 0.
    """
    if integer > 0:
        print "Higher than 0"
...
def is_divisible_by_three(integer):
    """
    Check if input integer is divisible by three.  Return True or False.
    """
    if integer % 3 == 0:
        return True
    else:
...
def total_meal_price(meal_price, tip_percentage = .15):
    """
    Return the total meal price based on the meal price and tip percentage.
    If no tip is provided, the default is 15%.
    """
...
def sign_and_parity(integer):
    """
    Determine if an integer is positive or negative, and even or odd.
    Return the resulting two strings in a ist.
    """
    if integer % 2 == 0 and integer > 0:
        parity_and_sign = ["Even", "Positive"]
    elif integer % 2 == 0 and integer < 0:
        parity_and_sign = ["Even", "Negative"]
    elif integer % 2 != 0 and integer > 0:
        parity_and_sign = ["Odd", "Positive"]
    else:
        parity_and_sign = ["Odd", "Negative"]
    return parity_and_sign

check_1 = sign_and_parity(5)
parity, sign = check_1[0], check_1[1]
print parity
print sign
...
def full_title(name, job_title = "Engineer"):
    """
    Return a person's job title and name.  If a job title is not provided,
    the default job title is Engineer.
    """
...
def write_letter(name, job_title, sender_name):
    """
    Print a letter, including recipient name, job title, and sender name.
    Call full_title function to construct the job title + recipient name.
    """
    full_title_greeting = full_title(name, job_title)
    print "Dear " + full_title_greeting + ", I think you are amazing! Sincerely, " + sender_name
